...
const QuizDisplay = ({ questions, onComplete }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`absolute left-${[position]}px bottom-0 mb-4 -translate-x-1/2 w-12 h-10 bg-white shadow-[0px_12px_30px_0px_rgba(16,24,40,0.1)] rounded-full px-3.5 py-2 text-gray-800 text-xs font-medium flex justify-center items-center after:absolute after:bg-white after:flex after:bottom-[-5px] after:left-1/2 after:-z-10 after:h-3 after:w-3 after:-translate-x-1/2 after:rotate-45`}>{currentQuestion + 1}</span>
      <div className='relative flex w-full h-2.5  overflow-hidden rounded-3xl bg-gray-100'>
      <div role='progressbar' aria-valuenow='50' aria-valuemin='0' aria-valuemax='100'  className='flex h-full items-center justify-center bg-indigo-600  text-white rounded-3xl' style={{
